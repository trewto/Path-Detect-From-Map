...
main_grid = [[{'x': row, 'y': col, 'is_blocked': False, 'is_visited': False, 'distance': float('inf') , 'previous' : None}
         for col in range(COLS)] for row in range(ROWS)]

for row in range(ROWS):
    for col in range(COLS):
            main_grid[row][col]['is_blocked']= grid[row][col]['is_blocked']
...
contour_grid_gap = 5
#this is is position in grid axis
def contour_positions(grid,contour_grid_gap):
    ROWS = len(grid)
    COLS = len(grid[0])

    temp_grid = [[{'x': row, 'y': col, 'is_blocked': True, 'is_visited': False, 'distance': float('inf') , 'previous' : None}
         for col in range(COLS)] for row in range(ROWS)]
    
    positions = []

    for row in range(0, ROWS, contour_grid_gap):
        for col in range(COLS):
            if grid[row][col]['is_blocked'] == False:
                temp_grid[row][col]['is_blocked'] = False
                positions.append((row, col))
                
            
    
    for col in range(0, COLS, contour_grid_gap):
        for row in range(ROWS):
            if grid[row][col]['is_blocked'] == False:
                temp_grid[row][col]['is_blocked'] = False
    ...
    for row in range(ROWS):
        for col in range(COLS):
                 grid[row][col]['is_blocked']= temp_grid[row][col]['is_blocked']
    
    return positions ; 

def is_contour_position(x, y, positions):
...
positions = contour_positions(grid,contour_grid_gap)
CC = len(positions)
RR = col
distance_matrix = [[float(-1) for col in range(CC)] for row in range(RR)]


def check_srrouding_point_in_contour_and_create_distance_matrix(x1,y1,positions,distance_matrix):
#testing 
#x1 = 35
#y1 = 45
    pos1 = is_contour_position(x1, y1, positions)
    for i in range( (x1//contour_grid_gap)*contour_grid_gap , (x1//contour_grid_gap)*contour_grid_gap + contour_grid_gap) :
        for j in range( -1, 2):
            if grid[i][y1+contour_grid_gap*j]['is_blocked'] == False:
                1==1 # make matrix and distance
                pos2 = is_contour_position(i, y1+contour_grid_gap*j, positions)
                distance_matrix[pos1][pos2] = distance_in_contour(x1,y1,i, y1+contour_grid_gap*j)
           
    #again 
    for i in range(0,2):
        for j in range( y1 -contour_grid_gap , y1 + contour_grid_gap):
            if grid[(x1//contour_grid_gap)*contour_grid_gap+i *contour_grid_gap][j]['is_blocked'] == False:
                1==1 # make matrix and distancce
                pos2 = is_contour_position((x1//contour_grid_gap)*contour_grid_gap+i *contour_grid_gap,j, positions)
                distance_matrix[pos1][pos2] = distance_in_contour(x1,y1,(x1//contour_grid_gap)*contour_grid_gap+i *contour_grid_gap,j)
            
         

def distance_in_contour(x1,y1,x2,y2):
    #by default distance inf
    1==1
